...
from critical_patching import apply_association_function # importing here OK due to use of monkeypatching, this reference requires __init__.py files
...
@mock_ssm
def test_apply_association_lambda_happy_path(state: TestState, monkeypatch):
    # setup
...
@mock_ssm
def test_apply_association_lambda_no_listed_associations_throws_exception(state: TestState, monkeypatch):
    # setup
    def ssmclient_list_associations_mock(AssociationFilterList: List) -> Dict:
        state.ssmclient_list_associations_call = AssociationFilterList
